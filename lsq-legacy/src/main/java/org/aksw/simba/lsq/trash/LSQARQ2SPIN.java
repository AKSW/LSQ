...
import org.aksw.simba.lsq.core.util.ReverseRenameUtils;
 ...
 *
 * Adaption for AKSW LSQ by Claus Stadler 2016
 */
public class LSQARQ2SPIN {
     ...
     */
    public LSQARQ2SPIN(Model model) {
        this(model, true);
    }
     ...
     */
    public LSQARQ2SPIN(Model model, boolean addPrefixes) {
        this.model = model;
    ...
    private RDFNode getNode(Node node) {
        node = ReverseRenameUtils.effectiveNode(node);
        if(node.isVariable()) {
